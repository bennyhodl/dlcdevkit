...
#[cfg(test)]
mod dlc_tests {
    use crate::tests::util::setup_bitcoind_and_electrsd_and_bella_wallet;
    #[tokio::test]
    async fn test_party_params() {
        let (_, _, wallet) = setup_bitcoind_and_electrsd_and_bella_wallet();
...
pub type BellaRuntime = Arc<RwLock<Option<tokio::runtime::Runtime>>>;

pub struct Bella {
    pub runtime: BellaRuntime,
    pub wallet: Arc<BellaWallet>,
}
...
pub fn build(name: String, esplora_url: String, network: Network) -> anyhow::Result<Bella> {
    let runtime = Arc::new(RwLock::new(None));

    let wallet = Arc::new(BellaWallet::new(
        name,
        esplora_url,
        network,
        runtime.clone(),
...
```
// Oracles
let mut oracles: Arc<HashMap<XOnlyPublicKey, BellaOracle>> = Arc::new(HashMap::new());
let oracle = BellaOracle::default();
