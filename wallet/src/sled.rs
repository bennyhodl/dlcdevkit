...
mod tests {
    ...
    macro_rules! sled_test {
        ($name: ident, $body: expr) => {
            #[test]
            fn $name() {
                let path = format!("{}{}", "test_files/sled/sleddb/", std::stringify!($name));
                {
    ...
    sled_test!(
        ...
        |storage: SledStorageProvider| {
            let serialized = include_bytes!("../test_files/sled/Offered");
    ...
    sled_test!(
        ...
        |storage: SledStorageProvider| {
            ...
            let serialized = include_bytes!("../test_files/sled/Accepted");
    ...
    fn insert_offered_signed_and_confirmed(storage: &mut SledStorageProvider) {
        ...
        let serialized = include_bytes!("../test_files/sled/Signed");
        let signed_contract = Contract::Signed(deserialize_object(serialized));
        storage
            .update_contract(&signed_contract)
            .expect("Error creating contract");
        let serialized = include_bytes!("../test_files/sled/Signed1");
        let signed_contract = Contract::Signed(deserialize_object(serialized));
        storage
            .update_contract(&signed_contract)
            .expect("Error creating contract");

        let serialized = include_bytes!("../test_files/sled/Confirmed");
        let confirmed_contract = Contract::Confirmed(deserialize_object(serialized));
        storage
            .update_contract(&confirmed_contract)
            .expect("Error creating contract");
        let serialized = include_bytes!("../test_files/sled/Confirmed1");
        let confirmed_contract = Contract::Confirmed(deserialize_object(serialized));
        storage
            .update_contract(&confirmed_contract)
            .expect("Error creating contract");

        let serialized = include_bytes!("../test_files/sled/PreClosed");
    ...
    fn insert_offered_and_signed_channels(storage: &mut SledStorageProvider) {
        let serialized = include_bytes!("../test_files/sled/Offered");
        let offered_contract = deserialize_object(serialized);
        let serialized = include_bytes!("../test_files/sled/OfferedChannel");
        ...
        let serialized = include_bytes!("../test_files/sled/SignedChannelEstablished");
        let signed_channel = Channel::Signed(deserialize_object(serialized));
        storage
            .upsert_channel(signed_channel, None)
            .expect("Error creating contract");

        let serialized = include_bytes!("../test_files/sled/SignedChannelSettled");
        let signed_channel = Channel::Signed(deserialize_object(serialized));
        storage
            .upsert_channel(signed_channel, None)
            .expect("Error creating contract");
    ...
    sled_test!(
        ...
        |mut storage: SledStorageProvider| {
            ...
            let serialized = include_bytes!("../test_files/sled/AcceptedChannel");
