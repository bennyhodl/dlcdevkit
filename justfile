...
node-one:
  - cargo run --bin ddk-node -- --network regtest --esplora http://127.0.0.1:30000 --name node-one --postgres-url postgres://loco:loco@localhost:5432/ddk_one

node-two:
  - cargo run --bin ddk-node -- --network regtest --esplora http://127.0.0.1:30000 --port 1777 --grpc 0.0.0.0:3031 --storage-dir ~/.ddk/node-two --name node-two --postgres-url postgres://loco:loco@localhost:5432/ddk_two
