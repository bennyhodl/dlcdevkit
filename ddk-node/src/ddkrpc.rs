...
pub mod ddk_rpc_server {
    ...
    #[async_trait]
    pub trait DdkRpc: std::marker::Send + std::marker::Sync + 'static {
        async fn info(
            &self,
    ...
    impl<T> Clone for DdkRpcServer<T> {
        fn clone(&self) -> Self {
            let inner = self.inner.clone();
            Self {
                inner,
    ...
    pub const SERVICE_NAME: &str = "ddkrpc.DdkRpc";
    impl<T> tonic::server::NamedService for DdkRpcServer<T> {
